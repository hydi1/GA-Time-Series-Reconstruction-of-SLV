...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            ...
            full_length = 48  #seq_len+num_test
            ...
            # 保存基于 batch 的反归一化数据到 Excel
            all_batch_y = np.concatenate(all_batch_y, axis=0).reshape(total_samples, -1)  # (37, 12)
            all_outputs = np.concatenate(all_outputs, axis=0).reshape(total_samples, -1)  # (37, 12)
            ...
            save_path_batch = r'D:\project\convgru_TSVU - 反归一化\output_test\output_y18.xlsx'
            results_df.to_excel(save_path_batch, index=False)

            # 保存去重叠后的反归一化数据到 Excel
            ...
            batch_y_df_full = pd.DataFrame(batch_y_full_unnormalized, columns=batch_y_columns)  # 形状 (1, 48)
            outputs_df_full = pd.DataFrame(outputs_full_unnormalized, columns=outputs_columns)  # 形状 (1, 48)
            results_df_full = pd.concat([batch_y_df_full, outputs_df_full], axis=0)  # 按行拼接，形状 (2, 48)
            save_path_full = r'D:\project\convgru_TSVU - 反归一化\output_test\output_y48_deoverlapped.xlsx'
            ...
            # 保存到 txt 文件
            save_path_txt = os.path.join(r"D:\project\convgru_TSVU - 反归一化\SOFTS-main\test_result", 'test_results.txt')
    ...
    #
    #         # 保存到 Excel 文件
    #         save_path = r'D:\project\convgru_TSVU\output_test\output_y18.xlsx'
    ...
    #     #保存到 txt 文件中
    #
    #     save_path = os.path.join(r"D:\project\convgru_TSVU\SOFTS-main\test_result", 'test_results.txt')
    #     # 将结果写入文件
