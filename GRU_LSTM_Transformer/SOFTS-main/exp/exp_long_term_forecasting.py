...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            all_outputs = []  # 用于保存所有预测结果
            all_batch_y = []  # 用于保存所有真实值
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # 计算基于 batch 的反归一化损失
                rmse_batch.update(rmse_loss(outputs_unnormalized_tensor, batch_y_unnormalized_tensor).item(),
                                  batch_x.size(0))
                mae_batch.update(mae_loss(outputs_unnormalized_tensor, batch_y_unnormalized_tensor).item(),
                                 batch_x.size(0))

                # 保存每个 batch 的输出用于去重叠
                all_outputs.append(outputs_cpu)  # [batch_size * seq_len, 1]
                all_batch_y.append(batch_y_cpu)  # [batch_size * seq_len, 1]
            ...
            save_path = r"D:\project\GRU - 反归一化\output_test\Transformer.xlsx"
        ...
        # 保存结果到 txt 文件
        save_path = os.path.join(r"D:\project\GRU - 反归一化\SOFTS-main\test_result", 'test_results.txt')
        with open(save_path, 'a') as f:
            if isinstance(setting, dict) and 'epoch' in setting:
                test_result = (f"Epoch: {setting['epoch']}, lr={self.args['learning_rate']}, "
                               f"Batch-based: rmse: {rmse_batch.avg}, mae: {mae_batch.avg}, "
                               f"Full-sequence: rmse: {rmse_full.avg}, mae: {mae_full.avg}")
            else:
                test_result = (f"lr={self.args['learning_rate']}, "
                               f"Batch-based: rmse: {rmse_batch.avg}, mae: {mae_batch.avg}, "
                               f"Full-sequence: rmse: {rmse_full.avg}, mae: {mae_full.avg}")
            f.write(test_result + '\n')
